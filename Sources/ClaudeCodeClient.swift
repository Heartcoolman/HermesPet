...
"""
    ...
    func streamCompletion(messages: [ChatMessage],
        ...
        return streamRaw(prompt: prompt, extraAddDirs: extraDirs, tier: tier)
    }

    /// 底层 spawn claude -p prompt 的流式实现
    /// `tier` 决定是否附加 `--model` —— balanced 时不传，沿用 CLI 自己的默认（最稳）。
    private func streamRaw(prompt: String,
        ...
        AsyncThrowingStream { continuation in
            ...
            // 自动档位：fast=haiku / deep=opus；balanced 时 tier.claudeModel == nil 不传 --model
            // 由 ModelTier.decide(messages:) 在 ChatViewModel 里按附件/关键词/长度判定，UI 不暴露
            if let model = tier.claudeModel {
                args.append("--model")
                args.append(model)
            }
