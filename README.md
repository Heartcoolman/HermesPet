...
子进程并行跑（最多 2 个同时活跃，剩下排队），互不干扰。切换对话时 header 的 mode 颜色和图标实时同步，灵动岛精灵也跟着切。
...
欢迎 PR！开干前建议先开 issue 聊聊方向。

**调试约定**：
- 用 `./install.sh` 部署，运行的是 `/Applications/Hermes 桌宠.app`，不是 `./HermesPet.app`
- 改完代码 `./build.sh 2>&1 | grep -E "error:|Build complete"` 快速验证
- macOS 26 + SwiftUI 对 layout cycle 极敏感 —— 跨窗口 setFrame 一律 `DispatchQueue.main.async`，详见 [CLAUDE.md](./CLAUDE.md) 决策 #5
- Swift 6 严格并发：大量后台回调的 controller 必须 `@unchecked Sendable` + NSLock，**不能用 `@MainActor` 类**

---

## 📋 路线图

详见 [TODO.md](./TODO.md)，下一阶段重点想做的：

- [ ] **AI 工作流编排** —— 把"Hermes 出方案 → Claude 实现 → Codex 出海报"做成可保存的链
- [ ] **工具调用透明化** —— Claude Read/Write/Bash 时气泡里能展开看 diff / 命令输出
- [ ] **跨对话搜索** —— ⌘F 全局搜历史（基于 SQLite FTS）
- [ ] **场景触发器** —— "切到 Xcode → Clawd 凑过来问要不要 review"
