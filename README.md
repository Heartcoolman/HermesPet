...
**Markdown** 完整渲染（GFM 表格 + 编号列表转可点卡片 + 代码块带"复制"）· **Pin 桌面卡片**把任意 AI 回答钉到桌面 · **快问浮窗** `⌘⇧Space` Spotlight 风不开聊天窗也能问 · **输入栏严格按 Apple HIG**（Capsule + iMessage 风）· **聊天字号 5 档可调**（`⌘+` / `⌘-` / `⌘0`）· **窗口置顶**随手切 · **Dock 图标可选**（默认菜单栏 agent 风格不占 Dock）· **5 个事件提示音**可独立开关 + 拖入自定义音频。
